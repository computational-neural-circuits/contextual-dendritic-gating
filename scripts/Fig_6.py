...
parameter_dict = {}
EMNIST_DATA_ROOT = Path(__file__).resolve().parents[1] / "results" / "datasets"


class ProjectEMNIST(datasets.EMNIST):
...
def Fig_6(
    ...
    use_opposite_context=False,
    use_variance_in_auditory=False,
):
    folder_name = f"Fig_6"
    if use_opposite_context:
        folder_name = f"Fig_S6"
    ...
    samples, patches = load_EMNIST_data_and_create_gabor_patches(
        target_labels,
        seed=seed,
        create_random_patch_positions=create_random_patch_positions,
    )
    ...
    for ii, ax in enumerate(ax_gabor.flatten()):
        ...
        try:
            to_show[
                x - patch_x // 2 : x + patch_x // 2, y - patch_y // 2 : y + patch_y // 2
            ] = patch
    ...
    fig_gabor.savefig(
        get_path_to_save_file_name(folder_name, "gabor_patches.pdf"), dpi=600
    )
    ...
    all_inputs = np.zeros(
        (len(target_labels), n_train + n_recall + n_train_additional, 400)
    )
    ...
    for ll, label in enumerate(target_labels):
        for ii in range(n_train + n_recall + n_train_additional):
            ...
            max_response = 12
            x = process_gabor_filtered_array_for_input(
                gabor, n=20, max_response=12
            ).flatten()
        ...
        this_ax.set(
            aspect=(n_train + n_recall + n_train_additional) / 400.0, title=label
        )
        ...
        this_ax.set(
            xticks=[ii for ii in range(n_train + n_recall + n_train_additional)],
            xticklabels=[],
        )
    ...
    for ll, label in enumerate(target_labels):
        for ii in range(n_train):
            ...
            auditory_assembly_id = auditory_map_for_labels[label]
            if use_variance_in_auditory:
                auditory_assembly_id = len(all_assembly_inputs) - 1
                if label in ["l", "O"]:
                    auditory_assembly_id += 1000
    ...
    for ii in range(n_recall_auditory):
        for ll, label in enumerate(target_labels):

            if not use_variance_in_auditory:
                all_assembly_inputs_recall.append([])
            else:
                all_assembly_inputs_recall.append(
                    all_assembly_inputs_recall[ii * len(target_labels) + ll]
                )
    ...
    if n_recall_both > n_recall:
        raise ValueError(
            "We use the same visual cue for simple recall and both, so both need to have the same value"
        )
    for ii in range(n_recall_both):
        for ll, label in enumerate(target_labels):
    ...
    all_assembly_inputs_key = hashlib.sha1(
        json.dumps(
            {"all_assembly_inputs": all_assembly_inputs}, sort_keys=True
        ).encode()
    ).hexdigest()[:12]
    ...
    if n_train_additional > 0:
        ...
        net.parameters_for_run["all_imprint_ids"] = (
            all_imprint_ids_for_additional_training
        )
        ...
        net.run_imprint(
            report_style="text",
            report_period=900 * second,
            all_assembly_inputs=all_assembly_inputs_for_additional_training,
        )
    ...
    for context_id in range(2):
        ...
        for area in net.all_areas:
            sorted_neuron_ids = net.sort_neurons_by_weights(
    ...
    for key, imprint_id in all_last_imprint_ids.items():
        ...
        start_time = (
            (imprint_id + 1) * (bsl + rtm) - rtm_rec + bsl * int(n_train_additional > 0)
        )
        ...
        actual_imprint_id = imprint_id - len(target_labels) * n_train * int(
            n_train_additional > 0
        )

        for area in net.all_areas:
            sorted_neuron_ids_for_imprint, _, assembly_neuron_ids = (
                net.sort_neurons_by_firing_rate(
                    area=area,
                    sort_for_specific_imprint=actual_imprint_id,
                    t_0=int(n_train_additional > 0)
                    * (len(target_labels) * n_train * (bsl + rtm) + bsl),
                )
            )
            sorted_last_imprint_ids.append(sorted_neuron_ids_for_imprint)
            assembly_neuron_ids_for_targets_and_all_areas[key].append(
                assembly_neuron_ids
            )
            ...
            last_imprint_n_active_for_targets_and_all_areas[key].append(
                n_active_neurons
            )
    ...
    net.save_weight_matrix_for_matlab(
        show_plot=False,
        matlab_export_name=get_path_to_save_file_name(folder_name, "weights"),
    )
    # n_recall, n_targets, assembly_id, areas, rate/n_active
    recall_results = np.zeros(
        (n_recall, len(target_labels), len(target_labels), 3, 2)
    ) * float("nan")

    recall_results_auditory = np.zeros(
        (n_recall_auditory, len(target_labels), len(target_labels), 3, 2)
    ) * float("nan")

    recall_results_both = np.zeros(
        (n_recall_both, len(target_labels), len(target_labels), 3, 2)
    ) * float("nan")
    ...
    for running_recall_number, (recall_inputs, recall_id) in enumerate(
        ...
        for run_recall_after_imprint in [True]:
            all_assembly_inputs_key_recall = hashlib.sha1(
                json.dumps(
                    {"all_assembly_inputs_recall": recall_inputs}, sort_keys=True
                ).encode()
            ).hexdigest()[:12]
            ...
            ll = running_recall_number % len(target_labels)
            ii = running_recall_number // len(target_labels)
            ...
            for area_id, area in enumerate(net.all_areas):
                for assembly_id, key in enumerate(target_key_list):
                    assembly_neuron_ids = assembly_neuron_ids_for_targets_and_all_areas[
                        key
                    ][area_id]

                    if assembly_id == ll:
                        assembly_ids_for_recall_for_all_areas.append(
                            assembly_neuron_ids
                        )
                    ...
                    if ii < n_recall:
                        recall_results[ii, ll, assembly_id, area_id, 0] = (
                            avg_firing_rate
                        )
                        recall_results[ii, ll, assembly_id, area_id, 1] = (
                            n_active_neurons
                        )
                    elif ii < n_recall + n_recall_auditory:
                        recall_results_auditory[
                            ii - n_recall, ll, assembly_id, area_id, 0
                        ] = avg_firing_rate
                        recall_results_auditory[
                            ii - n_recall, ll, assembly_id, area_id, 1
                        ] = n_active_neurons
                    else:
                        recall_results_both[
                            ii - n_recall - n_recall_auditory,
                            ll,
                            assembly_id,
                            area_id,
                            0,
            ...
            net.show_spike_rasters(
                ...
                highlight_neuron_ids=[
                    [ii, vals]
                    for ii, vals in enumerate(assembly_ids_for_recall_for_all_areas)
                ],
                ...
                save_file_name=get_path_to_save_file_name(
                    folder_name,
                    f"recall_spikes_for_recall_image_{target_key_list[ll]}_{ii}",
                ),
                only_show_spikes_after=first_last_imprint_start,
            )

    for name, res in zip(
        ["visual", "auditory"], [recall_results, recall_results_auditory]
    ):

        if name == "auditory" and n_recall_auditory == 0:
            continue
            ...
            for area_id in range(len(net.all_areas)):
                ...
                for target_id, target_name in enumerate(target_key_list):
                    ...
                    if n_recall_both == 0:
                        ...
                        y = recall_results_both[:, target_id, :, area_id, rate_n_active]
                        values_for_x = [
                        ...
                        for recall_id in range(n_recall_both):
                            if label_values is not None:
                                this_label = "recall with both" + f"{recall_id}"
                            ax.plot(
                                values_for_x,
                                y[recall_id, :],
                                color="k",
                    ...
                    y = res[:, target_id, :, area_id, rate_n_active]
                    values_for_x = [
                        target_id - 0.25 + (0.5 * ff / (len(target_labels) - 1))
                        for ff in range(len(target_labels))
                    ]
                    ...
                    if name == "auditory":
                        nn = n_recall_auditory

                    for recall_id in range(nn):
                        if label_values is not None:
        ...
        fig.savefig(
            get_path_to_save_file_name(folder_name, f"recall_results_{name}.pdf"),
            dpi=600,
        )
...
def filter_image_with_patches(image, patches, create_random_patch_positions):
    ...
    for idx, (patch, (x, y)) in enumerate(patches):
        ...
        if not create_random_patch_positions:
            response_grid[idx // 20, idx % 20] = np.sum(
                filtered_image[x - 2 : x + 2, y - 2 : y + 2]
            )
        else:
            response_grid[idx // 20, idx % 20] = np.sum(
                filtered_image[x - 1 : x + 1, y - 1 : y + 1]
            )
...
def load_EMNIST_data_and_create_gabor_patches(
    ...
    emnist_train = ProjectEMNIST(
        root=EMNIST_DATA_ROOT,
        split="byclass",
        train=True,
        download=True,
        transform=transform,
    )
    _emnist_test = ProjectEMNIST(
        root=EMNIST_DATA_ROOT,
        split="byclass",
        train=False,
        download=True,
            ...
            for orientation in all_orientations:  # , 90, 135]:
                for phase in all_phases:
                    for frequency in all_frequencies:
                        patch = gabor_patch(
                            image_size, frequency, sigma, orientation, phase
                        )
