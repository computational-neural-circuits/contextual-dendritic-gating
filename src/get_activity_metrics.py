...
def get_activity_metrics_from_assembly_neurons(
    ...
    time_window = end_time - (start_time + remove_fraction_of_start * (end_time - start_time))
    ...
    if select_randomly_for_background:
        avg_firing_rate_not_in_assembly = np.mean(firing_rates_not_in_assembly[: len(selected_ids)])
